...
func TestSuggestionAcceptAppliesUpdate(t *testing.T) {
	...
	specPath := filepath.Join(root, ".gurgeh", "specs", "PRD-001.yaml")
	specRaw := []byte("id: \"PRD-001\"\ntitle: \"Title\"\nsummary: \"Old\"\n")
	if err := os.WriteFile(specPath, specRaw, 0o644); err != nil {
		t.Fatal(err)
	}
	suggPath, err := suggestions.Create(filepath.Join(root, ".gurgeh", "suggestions"), "PRD-001", time.Date(2026, 1, 15, 0, 0, 0, 0, time.UTC))
	if err != nil {
		t.Fatal(err)
	}
