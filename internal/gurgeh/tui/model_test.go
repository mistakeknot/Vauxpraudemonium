...
func TestViewShowsFirstSpec(t *testing.T) {
	...
	spec := "id: \"PRD-001\"\ntitle: \"Alpha\"\nsummary: \"First\"\n"
	if err := os.WriteFile(filepath.Join(root, ".gurgeh", "specs", "PRD-001.yaml"), []byte(spec), 0o644); err != nil {
		t.Fatal(err)
	}
	cwd, err := os.Getwd()
	if err != nil {
...
market_research:
  - id: "MR-001"
    claim: "Market"
    evidence_refs:
      - path: ".gurgeh/research/PRD-001-20260115-000000.md"
        anchor: "section-1"
        note: "Source"
    confidence: "medium"
    date: "2026-01-15"
...
func TestKeyLaunchesResearchAndSetsStatus(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, ".gurgeh", "research"), 0o755); err != nil {
		t.Fatal(err)
	}
